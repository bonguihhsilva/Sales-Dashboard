...
const META_COLORS = ['#6b6f7a', '#c9a24b', '#2e4fe3', '#f5a742']

export default function EvolutionChart({ data, vendorName }: { data: VendorEvolution[]; vendorName: string }) {
  if (!data.length) {
    return (
      <div style={{ textAlign: 'center', color: 'var(--muted)', fontFamily: 'var(--font-jetbrains), monospace', fontSize: '0.85rem', padding: '3rem' }}>
  ...
  return (
    <div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', padding: '1.5rem', marginBottom: '1.25rem' }}>
        <div style={{ fontSize: '0.68rem', fontFamily: 'var(--font-jetbrains), monospace', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '1rem' }}>
          Evolução de Vendas — {vendorName}
        </div>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            <XAxis dataKey="name" tick={{ fill: '#6b6f7a', fontSize: 11, fontFamily: 'var(--font-jetbrains), monospace' }} axisLine={false} tickLine={false} />
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              {['Período', 'Total Vendido', '1ª Meta', '3ª Meta', 'Meta', 'Bônus', 'Clientes'].map(h => (
                <th key={h} style={{ fontFamily: 'var(--font-jetbrains), monospace', fontSize: '0.6rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '6px 10px', textAlign: h === 'Período' ? 'left' : 'right' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.map(d => {
              ...
              return (
                <tr key={`${d.year}-${d.month}`} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'right', fontFamily: 'var(--font-jetbrains), monospace', fontWeight: 700, color: col }}>{fmtCurrency(Number(d.total_sold))}</td>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                    <span style={{ fontFamily: 'var(--font-jetbrains), monospace', fontSize: '0.7rem', padding: '2px 8px', borderRadius: '4px', background: col + '22', color: col }}>
                      {lvl > 0 ? `${lvl}ª meta` : '—'}
                    </span>
                  </td>
                  <td style={{ padding: '8px 10px', textAlign: 'right', fontFamily: 'var(--font-jetbrains), monospace', color: col }}>{Number(d.bonus_earned) > 0 ? `+$${Number(d.bonus_earned)}` : '—'}</td>
